...
day4 =
    context "Day 4" $ do
        describe "parseLine" $ do
            it "should parse a begins shift row" $
                parseLine "[1518-10-15 23:59] Guard #1699 begins shift"
                `shouldBe`
                Right (Row (mkDate 1518 10 15 23 59) (ShiftStart (GuardId 1699)))

            it "should parse a wakes up row" $
                parseLine "[1518-06-07 00:39] wakes up"
                `shouldBe`
                Right (Row (mkDate 1518 6 7 0 39) WakeUp)

            it "should parse a falls asleep" $
                parseLine "[1518-11-03 00:36] falls asleep"
                `shouldBe`
                Right (Row (mkDate 1518 11 3 0 36) Asleep)
                
        describe "tagEvent" $
            it "tag events and partition them by guard" $
                partitionEvents <$> mapM parseLine (T.lines exampleInput)
                `shouldBe`
                Right (M.fromList 
                    [(GuardId 659,
                            [ Row {date = mkDate 1518 6 3 0 0, event = ShiftStart (GuardId 659)}
                            , Row {date = mkDate 1518 6 7 0 39, event = WakeUp}
                            , Row {date = mkDate 1518 9 25 0 10, event = Asleep}
                            , Row {date = mkDate 1518 9 27 0 30, event = Asleep}
                            , Row {date = mkDate 1518 10 3 0 35, event = WakeUp}
                            ])
                    ,(GuardId 1699,
                            [ Row {date = mkDate 1518 10 15 23 59, event = ShiftStart (GuardId 1699)}
                            , Row {date = mkDate 1518 11 3 0 36, event = Asleep}
                            ])])
        describe "sleepIntervals" $
            it "should calculate the sum of sleep intervals partitioned by guard" $
                M.map sleepIntervals . partitionEvents <$> mapM parseLine (T.lines exampleInput)
                `shouldBe` 
                Right (M.fromList [(GuardId 659,[Interval 692700 35]),(GuardId 1699,[])])
        describe "sleepyHead" $ do 
            it "tweedledee slept the most" $
                sleepyHead . M.map sleepIntervals . partitionEvents <$> mapM parseLine (T.lines exampleInput)
                `shouldBe` 
                Right (GuardId 659) 
            it "tweedledee slept n hours" $
                do { m <- M.map sleepIntervals . partitionEvents <$> mapM parseLine (T.lines exampleInput) ;
                     pure $ maximum $ L.map duration $ m ! sleepyHead m }
                `shouldBe` 
                Right 692700
